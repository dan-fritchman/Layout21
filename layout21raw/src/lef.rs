...
// Local imports
use crate::{
    Abstract, AbstractPort, ErrorContext, HasErrors, LayerKey, LayoutError, LayoutResult, Library,
    Point, Shape, Units,
};
use lef21;
...
impl<'lib> LefExporter<'lib> {
    ...
    fn export_lib(&mut self) -> LayoutResult<lef21::LefLibrary> {
        self.ctx_stack
            .push(ErrorContext::Library(self.lib.name.clone()));
    ...
    fn export_abstract(&mut self, abstrakt: &Abstract) -> LayoutResult<lef21::LefMacro> {
        self.ctx_stack
            .push(ErrorContext::Cell(abstrakt.name.clone()));
